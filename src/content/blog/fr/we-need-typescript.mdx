...
Certain(e)s sont déjà convaincu(e)s par Typescript. Cet article ne va donc rien
vous apprendre, mais beaucoup ici sont aussi à la recherche de réponses. De
...
quelques moment où même Typescript ne pourra RIEN pour toi. C'est parti!
...
démarrrer ce nouveau cycle de deux semaines en choisissant une tâche où tu dois
restaurer le panier sauvegardé d'un utilisateur.

Tout se passe bien. À un moment, tu décides de coder une fonction qui permet
d'afficher le total du rabais de tous les élements du panier qui a été
sauvegardé, genre:
...
alerte sur Datadog. Une erreur Javascript a été détectée plus de 15000 fois sur
le site en production! Cette erreur est due au fait que le discount de certain
panier affiche `NaN` (Not A Number), ce qui est très frustrant pour les clients.

Après investigation, tu te rends compte que certains éléments du catalogue
d'articles ont une valeur `null` sur le discount. Comme tu es un 10x engineer,
tu envoies un fix super rapidement après la détection.
...
> Oui, il y a un milliard de manières pour résoudre ce problème, manières qui ne
> passent pas forcément par du code, mais laissez moi faire un exemple -
> merdique - en paix pour prouver mon point.
...
comme une indication, un contrat que ton code doit respecter pour espérer
...
programme appelé `tsc`), afin de vérifier si ton programme respecte bien les
contrats que tu as établi dans le code. Si jamais quelque chose semble
...
fait qu'un outil comm `tsc` va vérifier que les types et les usages des types de
ton programme sont correct **AVANT** que ton application tourne en production
(ce qu'on appelle le `runtime`). Typescript fait un job assez décent sur ce
point. Toute une classe de bugs est minimisée grâce à cette pratique.

Aussi, le code est d'un coup plus clair à lire et à comprendre, malgré l'absence
de documentation et de commentaire.
...
C'est une des grosses limites de Typescript, une fois que l'application tourne,
et fait appel à du code que l'on ne maitrise pas, on est de nouveau exposé aux
types d'erreurs que l'on souhaite réduire. La solution ici, c'est d'implémenter
une système de **runtime type checking**.

L'idée, c'est de créer une fonction, un validateur, qui va check tout ce qui
...
Ici j'utilise `Zod`. J'ai un validateur qui va vérifier mes `cartItem` au
runtime, quand ces items viennent de sources non-sûres comme le localStorage. Le
validateur me permet aussi de génerer un type Typescript gràce aux utilitaires
que `Zod` fournit. Pas mal non?
...
`any` en fait parti. C'est une manière de désactiver le checking en disant
qu'une variable peut être "n'importe quoi".

### Le tooling

Image que tu veuilles faire une webapp. Tu te retrouves à installer Typescript,
son tooling et comprendre sa configuration! Je dois admettre que c'est parfois
le plus gros point de difficulté. La configuration de Typescript est une étape
qui frustre énormément de gens.
...
Les avantages surpassent tellement les inconvénients. J'ai fait le parti de
