...
tags: [gleam, beam, programmation, functional]
...
<Image
  src={gleamBanner}
  alt="Étoile rose qui représente le logo de Gleam, suivit du texte 'Gleam'"
/>
...
<Image
  src={thingsToLearn}
  alt="Image qui représenter le classement des choses sur lesquelles j'aimerais garder un oeil, et Gleam est catégorisé non pas comme urgent mais comme potentiellement intéréssant."
/>
...
`Vehicule` est le nom du type, `Car`, `Skateboard` et `Spaceship` sont des
constructeurs disponibles pour ce type. Pour t'aider, tu peux voir `Vehicule`
...
```gleam
fn get_driving_requirements(vehicule: Vehicule) -> String {
  case vehicule {
    Car(make, brand) ->
      "To drive the car "
      <> make
...
1. `get_driving_requirements` prend en paramètre un seul argument de type
   `Vehicule` et retourne un `String`. Le compiler (et le LSP) de Gleam feront
   en sorte que ces conditions soient respectées. Le type de retour est
   optionnel, dans le sens où, si tu ne le mets pas, Gleam saura quand même ce
   que tu retournes.
2. Dans le corps de la fonction, on voit la structure `case`, connue sous le nom
   ...
   TOUTES les possibilités de valeur pour `vehicule` avec un niveau assez fou de
...
/// This is some type related to type Vehicule
type Vehicule {
...
Si je passe ma souris (hover action) sur la variable `vehicule` dans le corps de
ma fonction, je vois:

<Image
  src={lspExample1}
  alt="Screenshot qui montre que lorsqu'on passe la souris sur la variable vehicule, l'editeur de code est capable de montrer la documentation et le type relatif à la variable, grâce au LSP."
/>
...
Avec tout ça, tu te retrouves avec un nouveau process, stateful, a qui tu peux
...
function get_driving_requirements(vehicule) {
  if (vehicule instanceof Car) {
    let make = vehicule.make;
    let brand = vehicule.brand;
    return (
      "To drive the car " + make + " " + brand + ", your need a driver licence."
    );
  } else if (vehicule instanceof Spaceship) {
